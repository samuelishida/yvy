...
describe("fire_classify", function()
    ...
    describe("classify_fire", function()
        ...
        it("CAR + autorização (objeto) → evidence.authorization carrega nro/modo", function()
            local cfg = { sinaflor = function()
                return { nro = "ASV-123", modo = "ASV", data_inicio = "2026-01-01", data_fim = "2026-12-31" }
            end }
            local r = classify.classify_fire(fire(), { car = { name = "F", id = "1" } }, cfg)
            assert.are_equal("permitido", r.nature)
            assert.are_equal("ASV-123", r.evidence.authorization.nro)
            assert.are_equal("ASV", r.evidence.authorization.modo)
            assert.are_equal("2026-12-31", r.evidence.authorization.data_fim)
        end)
        it("CAR + hook retornando false → suspeito (sem autorização)", function()
            local cfg = { sinaflor = function() return false end }
            local r = classify.classify_fire(fire(), { car = { name = "F", id = "1" } }, cfg)
        ...
        it("NATURE_VERSION default = 3", function()
            assert.are_equal(3, classify.NATURE_VERSION)
        end)
    end)
end)
