...
describe("car routes", function()
    ...
    describe("/api/car/lookup", function()
        ...
        it("tolerance snap reports source=snap and distance_m", function()
            -- Ligeiramente fora de MT-1 a oeste (-55.0015 ≈ 163 m), dentro de 200 m
            local ctx = fake_ctx({ lat = "-12.5", lon = "-55.0015", tolerance = "200" })
            car_routes.get_lookup(ctx)
            assert.are_equal(200, ctx.status)
            assert.is_not_nil(ctx.body.imovel)
            assert.are_equal("MT-1", ctx.body.imovel.id)
            assert.are_equal("snap", ctx.body.imovel.source)
            assert.is_true(ctx.body.imovel.distance_m <= 200)
        end)
        it("tolerance=5000 clamped to 2000 and still snaps", function()
            -- Fora de MT-1 por ~1.85 km; clamped a 2000 ainda acha.
            local ctx = fake_ctx({ lat = "-12.5", lon = "-55.017", tolerance = "5000" })
            car_routes.get_lookup(ctx)
            assert.are_equal(200, ctx.status)
            assert.is_not_nil(ctx.body.imovel)
            assert.is_true(ctx.body.imovel.distance_m <= 2000)
            assert.are_equal("snap", ctx.body.imovel.source)
        end)
        it("tolerance=-100 behaves like exact lookup", function()
            -- -55.0015 é snap com tolerance 200; com -100 deve cair no exact e retornar nil
            local ctx = fake_ctx({ lat = "-12.5", lon = "-55.0015", tolerance = "-100" })
            car_routes.get_lookup(ctx)
            assert.are_equal(200, ctx.status)
            assert.are_equal(cjson.null, ctx.body.imovel)
        end)
