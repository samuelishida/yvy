...
-- Port of backend/backend.py fire-related routes
...
function _M.get_fires()
    ...
    redis.set(cache_key, response, 60)  -- 1 min TTL
...
function _M.fetch_firms_data(global_sync)
    if FIRMS_MAP_KEY == "" then
        ngx.log(ngx.WARN, "FIRMS_MAP_KEY not configured, skipping fire data sync")
        return 0
    end
    ...
    for _, bbox in ipairs(bboxes) do
        ...
        ngx.log(ngx.INFO, "Fetching FIRMS fire data: ", url)

        local res, err = httpc:request_uri(url, {method = "GET"})
        if not res then
            ngx.log(ngx.ERR, "FIRMS fetch error: ", err)
            goto continue
        end

        if res.status ~= 200 then
            ngx.log(ngx.ERR, "FIRMS API returned ", res.status)
            goto continue
        end
        ...
        if #fire_docs > 0 then
            db.bulk_upsert_fires(fire_docs)
        end
    ...
    ngx.log(ngx.INFO, "FIRMS sync complete: ", total_count, " records")
    return total_count
end

-- ── POST /api/fires/sync ─────────────────────────────────────────────────

function _M.sync_fires()
    auth.enforce()
    rl.enforce()

    local args = ngx.req.get_uri_args()
    local global_sync = args["global"] == "1"

    local count = _M.fetch_firms_data(global_sync)

    ngx.header["Content-Type"] = "application/json"
    ngx.say(cjson.encode({status = "synced", records = count, global = global_sync}))
end

-- ── POST /api/admin/firms/sync ───────────────────────────────────────────

function _M.admin_firms_sync()
    auth.enforce()
    rl.enforce()

    ngx.log(ngx.INFO, "Manual FIRMS sync triggered")
