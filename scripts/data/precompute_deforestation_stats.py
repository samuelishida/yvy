...
Usage:
    python3 scripts/data/precompute_deforestation_stats.py --db backend-lua/data/yvy.db
"""
import argparse
import json
import os
...
def load_territories(kind: str):
    ...
    if kind == "municipio":
        ...
        if "nome" not in gdf.columns:
            gdf["nome"] = gdf["key"]
        # Explicit column-presence checks: `gdf.get("uf") or ...` on a Series
        # raises ValueError (ambiguous truth value). Missing UF column -> None
        # (municipality rows skipped from UF stats, no crash).
        if "uf" in gdf.columns:
            uf_col = "uf"
        elif "sigla_uf" in gdf.columns:
            uf_col = "sigla_uf"
        elif "estado" in gdf.columns:
            uf_col = "estado"
        else:
            uf_col = None
        gdf["uf"] = gdf[uf_col] if uf_col is not None else None
    elif kind == "uc":
