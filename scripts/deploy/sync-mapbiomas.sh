#!/usr/bin/env bash
# sync-mapbiomas.sh — gera o mapbiomas_alerta.db local (se velho/ausente), faz
# scp para produção e verifica.
#
# Fonte: MapBiomas Alerta (alerta.mapbiomas.org) — download em massa por estado.
# Fluxo (plan: risk-intelligence, Inc 1):
...
#
# Uso: bash scripts/deploy/sync-mapbiomas.sh [--vm-ip IP] [--force] [--dry-run]
set -euo pipefail
...
if [[ "$DRY_RUN" == false || ! -f "$DB" ]]; then
  run python3 "$PROJECT_DIR/scripts/data/download_mapbiomas_alerta.py" "${PY_ARGS[@]}"
fi
if [[ ! -f "$DB" ]]; then
